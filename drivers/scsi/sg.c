...
#define SG_KICK_FREQ 1000		/* time to loop before kicking block scheduler */
...
static atomic_t sg_count_reused_bios;
...
struct sg_slice_hdr4 {	/* parts of sg_io_v4 object needed in async usage */
	...
	u16 cmd_len;		/* truncated from sg_io_v4::request_len */
	u16 max_sb_len;		/* truncated from sg_io_v4::max_response_len */
	u16 mrq_ind;		/* mrq submit order, origin 0 */
	u16 nr_segs;		/* number of segments in bio (<= num_sgat) */
...
sg_wait_any_mrq(struct sg_fd_pollable *sfp_p, struct sg_mrq_hold *mhp, struct sg_request **srpp)
{
	...
	if (hipri) {
		...
		do {
			...
			if (++k == SG_KICK_FREQ) {
				k = 0;
				blk_mq_run_hw_queues(fp->parentdp->device->request_queue, true);
			}
		} while (true);
		*srpp = srp;
		return 0;
...
sg_svb_err_process(struct sg_mrq_hold *mhp, int m_ind, struct sg_fd *fp, int res, bool rs)
{
	...
	if (res) {
		...
		__maybe_unused struct sg_io_v4 *hp = mhp->a_hds + m_ind;
...
sg_mrq_ioreceive(struct sg_fd_pollable *sfp_p, struct sg_io_v4 *cop, void __user *p)
{
	...
	struct sg_mrq_hold mh;
	struct sg_mrq_hold *mhp = &mh;
	__maybe_unused struct sg_fd *fp = sfp_p->fp;
 ...
 * the same as old size; otherwise create a new sg_scatter_hold object and swap it with existing
 * reserve request's sg_scatter_hold object. Only modify first reserver request.
 */
static int
...
sg_start_req(struct sg_request *srp, struct sg_comm_wr_t *cwrp, int dxfer_dir)
{
	...
	if (no_dxfer) {
		...
		if (test_bit(SG_FRQ_LT_REUSE_BIO, srp->frq_lt_bm)) {
			if (srp->bio) {		/* 2,3,4 ... reuse bio handling */
				...
				/* used blk_rq_append_bio() before */
				blk_rq_bio_prep(rqq, srp->bio, srp->s_hdr4.nr_segs);
				atomic_inc(&sg_count_reused_bios);
	...
	if (likely(md)) {	/* normal, "indirect" IO */
		if (unlikely(rq_flags & SG_FLAG_MMAP_IO)) {
			...
			res = sg_mk_sgat(srp->sgatp, sfp, max_t(int, dlen, sfp->sgat_elem_sz));
		}
		if (unlikely(res))
			goto fini;
		md->pages = schp->pages;
...
sg_proc_seq_show_debug(struct seq_file *s, void *v, bool reduced)
{
	...
	if (it && it->index == 0)
		seq_printf(s, "max_active_device=%d  def_reserved_size=%d  num_reused_bios=%d\n",
			   ...
			   atomic_read(&sg_count_reused_bios));
